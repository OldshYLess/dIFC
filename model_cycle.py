...
class Discriminator(nn.Module):
    def __init__(self, input_size):
        ...
        #changed here!!!!!! 16 to 32, 64 to 128!!!!!
        assert input_size in [32, 128]
        self.blocks = [128, 128, 256, 256, 512, 512]
        # modified here!!!!!!! can we change 4 to 8, 2 to 4???????----------------
        pool_start = len(self.blocks) - 4 if input_size == 128 else len(self.blocks) - 2
...
class High2Low(nn.Module):
    def __init__(self):
        ...
        # noise layer, fully connected layer, 32*4=128
        self.noise_fc = nn.Linear(128, 16384)
    ...
    def forward(self, x, z):
        ...
        #print("-=-----------------------",z)
        #change noise from 64 64 to 128 128
        noises = noises.view(-1, 1, 128, 128)
        #print("xxx",x.shape)
        #print("noise",noises.shape)
...
def discriminator_test():
    in_size = 128 # 16, 64 !!! changed here 64 to 128!!!!!!
...
def high2low_test():
    net = High2Low().cuda()
    Z = np.random.randn(1, 1, 128).astype(np.float32)
    X = np.random.randn(1, 3, 128, 128).astype(np.float32)
    Z = torch.from_numpy(Z).cuda()
    X = torch.from_numpy(X).cuda()
    Y = net(X, Z)
    print("High2Low:",Y.shape)
